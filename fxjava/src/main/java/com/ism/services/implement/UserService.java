...
public List<User> getAllActifs(int type, User userConnect) {
    return userRepository.selectAllActifs(type)
            .stream()
            .filter(us -> us != null && !Objects.equals(us.getId(), userConnect.getId())) // Vérifier que us n'est pas null
...
public List<User> getAllActifs(int type) {
    return userRepository.selectAllActifs(type)
            .stream()
            .filter(us -> us != null && us.getClient() != null)  // Vérifier que us n'est pas null
            .collect(Collectors.toList());
}
